...
(function (global) {
  ...
  const PINYIN_CORRECTIONS = {
    ...
    "於": "yú",
    // 「行銷」單獨查沒問題，但句子裡若前面接著「知道」（知道 + 行銷部），
    // pinyin-pro 的斷詞會貪婪比對到「道行」這個固定詞（dào heng，指法力／功力），
    // 導致「行」被讀成 heng。加進 customPinyin 字典後，「行銷」比對優先度會提高，
    // 斷詞就會正確切成「知道」＋「行銷部」，不會被「道行」搶走。
    "行銷": "xíng xiāo",
    // 「嚇」套件預設讀成 hè（辭典正音，用於恐嚇、恫嚇等正式詞），但本專案的用法
    // 都是「嚇一跳」這種日常口語「受到驚嚇」的意思，現代口語一律讀 xià。
    "嚇": "xià",
    // 「還」套件預設讀成 huán（歸還、償還），但本專案的用法都是副詞「還／也／仍然」
    // 的意思（還以為、還需要、還有、還是、還沒……），這個意思一律讀 hái。
    "還": "hái",
  };
