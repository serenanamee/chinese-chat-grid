...
- 已知需要人工校正的字詞（透過 `pinyinPro.customPinyin()` 覆寫，見 `pinyin.js`）：
  ...
  - 「行銷」：單獨查沒問題，但句中若前面接「知道」（知道 + 行銷部），斷詞會被貪婪比對成固定詞「道行」（dào heng），導致「行」誤讀成 heng；加進字典後斷詞優先度提高即可修正
  - 「嚇」：套件預設讀辭典正音 hè（用於恐嚇、恫嚇等正式詞），但本專案的用法都是「嚇一跳」這種口語「受到驚嚇」的意思，應讀 xià
  - 「還」：套件預設讀成 huán（歸還、償還），但本專案的用法都是副詞「還／也／仍然」的意思（還以為、還需要、還有、還是、還沒……），應讀 hái
